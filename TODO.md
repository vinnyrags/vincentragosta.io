## HIGH PRIORITY
- populate staging WP cards catalog — staging has 0 cards (prod has many). Currently the homepage's Link prefetch to /cards 404s on staging, surfaced by the new Playwright error capture. Smoke test ignores this with a `/staging\.itzenzo\.tv\/cards/` regex but the actual fix is to push at least one card via `node scripts/shop/push-cards.js` against staging WP REST URL with `sk_test_*`.
- fix CI workflow PHPUnit job — has been failing on every push since `548c3ee` (workflow added). Error: `Trait "IX\Tests\Support\HasContainer" not found in tests/Integration/Providers/ProjectProviderTest.php on line 15`. Root cause documented in CLAUDE.md → "Stale IX/Mythus vendor after composer update". Fix: add `composer install --working-dir=../ix` (and likely `../../mu-plugins/mythus`) as CI steps in `.github/workflows/test.yml` before `composer test`. ~5 min fix.
